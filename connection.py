...
#         RLListItem = None
    ...
    with st.expander(f"{ward} 口服管制藥品查核"):
        ...
        if used_any:
            # 1️⃣ 使用 st.form 包裹輸入區域
            with st.form(key=f"{ward}_oral_form", clear_on_submit=True):
                st.subheader("💊 新增口服藥品使用紀錄")
                
                col1, col2 = st.columns(2)
                with col1:
                    current_drug = st.selectbox("選擇查核藥品", oral_drugs)
                    current_bed = st.text_input("床號(填床號數字就好)")
                    current_mrn = st.text_input("病歷號")
    
                with col2:
                    current_expected = st.number_input("應剩餘量", min_value=0, step=1)
                    current_actual = st.number_input("實際剩餘量", min_value=0, step=1)
                    current_reason = st.text_area("不符合原因 (若符合則免填)")
    
                # 2️⃣ 表單專用的提交按鈕
                submit_button = st.form_submit_button(f"➕ 添加 {current_drug} 查核紀錄")
                ...
                if submit_button:
                    if not current_bed or not current_mrn:
                        ...
                        new_record = {
                            ...
                            "不符合原因": current_reason if not match else "",
                        }
                        ...
                        # 因為 clear_on_submit=True，按下後欄位會自動清空，
                        # 且因為是在 Form 內，不需要手動 st.rerun() 也能更新下方列表。
            ...
            if "oral_data_records" in st.session_state and st.session_state.oral_data_records:
                ...
                if st.button("🗑️ 清空所有口服紀錄", key=f"{ward}_clear_oral"):
                    st.session_state.oral_data_records = []
                    st.rerun() # 這裡可以用 rerun，因為它不是在修改元件的 key
            else:
                st.info("💡 目前沒有紀錄。")
